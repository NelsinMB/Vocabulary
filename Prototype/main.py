...
def add_deck(name, context):
    db = DBManager()
    db.execute("INSERT INTO decks (name, parent) VALUES (?, ?)", (name, context.current_deck_id))
    print(f"Deck '{name}' added under deck ID {context.current_deck_id or 'root'}") # Review
    
def move_to_deck(name, context):
    db = DBManager()
    result = db.execute(
        "SELECT id FROM decks WHERE name = ? AND parent IS ?", 
...
def add_object(term, context):
    pass

def list_current_deck(context):
    pass
...
def parse_args(args_list=None):
    ...
    # We will have 'add deck' and 'add object'
    ...
    add_obj = add_subparsers.add_parser("object", help="Add a new object")
    add_obj.add_argument("term", help="The term (e.g., a word or concept)")
...
def handle_args(args, context=None):
    ...
    if args.command == "add":
        ...
        elif args.add_command == "object":
            add_object(args.term, context)
        else:
            print("You must specify to add either a deck or object")
